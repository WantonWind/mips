...
private:
	...
	atomic<int> regulator_prepared[4];	// 2表示所有的都已经做完了,最后一个用于全局判断
	...
	void prepare() {
		while (true) {
			...
			regulator_prepared[1] = true;		//保证输入的连续性??
		}
		while (regulator_prepared[1]) this_thread::yield();
	...
	void execution() {
		while (true) {
			...
			regulator_prepared[2] = true;		//保证输入的连续性??
		}
		while (regulator_prepared[2]) this_thread::yield();
	...
	void mem_access() {
		while (true) {
			...
			regulator_prepared[3] = true;		//保证输入的连续性??
		}
		while (regulator_prepared[3]) this_thread::yield();
...
ins string_to_ins(const string& s) {
	...
	int op;				//操作类型+功能
	int rs = -1;		//第一个操作数的寄存器地址
	int rt = -1;		//第二个操作数的寄存器地址（当第二个数不存在或者为立即数的时候默认为-1）
	int rd = -1;				//结果寄存器地址（-1代表low和high）
